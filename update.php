...
if(!empty($_GET["how_often"]) 		&& ($_GET["how_often"]==2)) $how_often = 2;
elseif(!empty($_GET["how_often"]) 	&& ($_GET["how_often"]==1)) $how_often = 1;
else 															$how_often = 4;

$MyMail = new MyMail($_SERVER['SERVER_NAME'], 25, "localhost", SUPPORT, EMAIL_PASS, "BUKETtime.ru");

$query = mysql_query('SELECT * FROM order_collection WHERE accept_in_chronopay=1 AND on_off=1 AND how_often=' . $how_often);
while($data = mysql_fetch_array($query)){
	...
	if($err != "")
		mail(EMAIL_CREATOR, $_SERVER['SERVER_NAME'] . ", Cron update.php: ошибка в curl у " . $customer_id, "Результат: " . $response);
	else{
